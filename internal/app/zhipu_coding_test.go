...
func TestZhipuCodingModelSelectionAndMenus(t *testing.T) {
	t.Parallel()
	providers := defaultProviders()
	configuration := config.Config{Provider: "zhipu-coding", ZhipuCodingAPIKey: "test-key"}
	...
	for _, option := range login.Options[1].Menu.Options {
		if option.Arguments == "zhipu-coding" {
			found = true
	...
	if got := len(runner.modelMenu().Options); got != 2 {
	...
	configuration.ContextWindows = map[string]int64{"zhipu-coding/glm-5.3": 1048576}
