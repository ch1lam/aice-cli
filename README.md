...
- Run `aice` (or `node bin/dev.js`) with no args to open the Ink UI (requires a real TTY). On first run it prompts for API key/model, writes `.env`, and validates connectivity. Slash commands (prefixed with `/`) handle help, login, opening the model menu, and starting a new session; plain input sends messages.
