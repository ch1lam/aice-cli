...
使用智谱时，在 API Key 登录菜单选择 `Zhipu API`。详见
...
| Provider | DeepSeek V4、OpenCode Go 内建模型目录、Kimi Coding Plan（Responses API）、Moonshot 开放平台 API、智谱开放平台 API、OpenAI GPT-5.6 API、Codex/ChatGPT 订阅，以及 Custom（OpenAI 兼容） |
...
provider-neutral；当前内建 provider 为 DeepSeek、OpenCode Go、Kimi Coding Plan、Moonshot API、智谱 API、OpenAI、Codex 与
Custom。
